...
_FORTRAN_BINODAL = "unset"   # cache: callable, or None if unavailable
def _get_fortran_binodal():
    """Lazily import the compiled f2py binodal module; return subregular_f(T,bulk,P) or None."""
    global _FORTRAN_BINODAL
    if _FORTRAN_BINODAL == "unset":
        try:
            import binodal_f
            _FORTRAN_BINODAL = getattr(binodal_f, "subregular_f", None) \
                or binodal_f.binodal_mod.subregular_f
        except Exception as _e:
            print(f"[use_fortran_binodal] compiled module unavailable ({_e}); using Python solver")
            _FORTRAN_BINODAL = None
    return _FORTRAN_BINODAL
...
def Planet_LAB(
    ...
    _eos_selftest=False,
    use_eos_cache=True,
    use_binodal_cache=True,
    use_fortran_binodal=False,
    snapshot=False,
    snap_X_int=None,          # bulk melt H2 MASS fraction (non-equilibrium interior)
    snap_T0_contact=None,     # boundary contact temperature T0 (K)
    snap_Ps_GPa=None,         # surface pressure (GPa); if None, root-find on snap_Matm_kg
    snap_Matm_kg=None,        # target atmosphere mass (kg) when snap_Ps_GPa is None
    snap_Msil_kg=None,        # silicate/core mass (kg); default Mp_target
    snap_force_Rcore_m=None,  # DEBUG: override core radius used for atm geometry
    snap_core_only=False,     # solve ONLY the core (fast) -> r_core; skip the atmosphere
):
...
# -------------- FUNCTIONS FOR MgSiO3 melt EOS ------------- #
    ...
    def compute_T0_potential_from_surface(Psurf, rho_0, Tsurf, wtfracH2=0.0):
        ...
        rho_surf_SI = _EOS_MgSiO3_exact(Psurf, rho_0, Tsurf, wtfracH2)  # kg/m^3 (one-shot: exact)
    ...
    def _EOS_MgSiO3_exact(P, rho_0, T0, wtfracH2=0.0):
        """
    ...
    # ---------- cached spline wrapper for MgSiO3 melt density ---------- #
    _eos_spline_cache = {}
    def EOS_MgSiO3(P, rho_0, T0, wtfracH2=0.0):
        if not use_eos_cache:
            return _EOS_MgSiO3_exact(P, rho_0, T0, wtfracH2)
        # eta(P) depends ONLY on (T0, wtfracH2); rho_0 is a linear scale
        # (rho = rho_0 * eta). Spline ln(eta) vs ln(P) once per (T0, wtfracH2).
        from scipy.interpolate import PchipInterpolator
        key = (round(float(T0), 1), round(float(wtfracH2), 7))
        sp = _eos_spline_cache.get(key)
        if sp is None:
            Pg = np.logspace(1.0, 16.0, 340)
            eta_g = np.array([_EOS_MgSiO3_exact(Pj, 1.0, T0, wtfracH2) for Pj in Pg]) / 1000.0
            sp = PchipInterpolator(np.log(Pg), np.log(eta_g), extrapolate=True)
            _eos_spline_cache[key] = sp
        Pin = np.asarray(P, dtype=float)
        # clamp to a positive floor: nonphysical (<=0) central-pressure probes
        # from Newton's root search must not produce NaN (which would force the
        # slow exact fallback). Real structure pressures are >> this floor.
        Pc_lo = np.maximum(Pin, 1.0e-2)
        with np.errstate(invalid='ignore'):
            eta = np.exp(sp(np.log(Pc_lo)))
        bad = ~np.isfinite(eta)
        if np.ndim(Pin) == 0:
            if bad:
                return _EOS_MgSiO3_exact(float(Pin), rho_0, T0, wtfracH2)
            return float(rho_0 * eta * 1000.0)
        if bad.any():
            for jx in np.where(bad)[0]:
                eta[jx] = _EOS_MgSiO3_exact(float(Pin[jx]), 1.0, T0, wtfracH2) / 1000.0
        return rho_0 * eta * 1000.0
    ...
    def _subregular_exact(Tsubreg, bulk_wtpercentH2, PGPa_sub, last_guess=None, lowT_cutoff=0.7):
        """
        ...
        # --- optional Fortran fast path (validated vs this solver; see binodal_f.f90) ---
        if use_fortran_binodal:
            _fb = _get_fortran_binodal()
            if _fb is not None:
                _T = Tsubreg if (Tsubreg is not None and np.isfinite(Tsubreg)) else 1500.0
                x1, x2, Tcrit, ws, wa = _fb(float(_T), float(max(bulk_wtpercentH2, 1.0e-3)),
                                            float(PGPa_sub))
                return float(x1), float(x2), float(Tcrit), float(ws), float(wa)
    ...
    # ---------- cached coexistence (binodal) wrapper ----------
    # (x1, x2, Tcrit) depend only on (T, P); bulk enters only via the analytic
    # lever rule (identical formula in both solver branches, MW1=2.02, MW2=100.3).
    # Memoize the expensive coexistence solve on (T, P); recompute lever rule per call.
    _coex_cache = {}
    def _lever_rule(x1, x2, bulk_wtpercentH2):
        MW1 = 2.02; MW2 = 100.3
        Rmass1 = (MW1/MW2)*(x1/(1.0-x1)); wtpercent1 = 100.0*(Rmass1/(Rmass1+1.0))
        Rmass2 = (MW1/MW2)*(x2/(1.0-x2)); wtpercent2 = 100.0*(Rmass2/(Rmass2+1.0))
        delta = wtpercent2 - wtpercent1
        return (wtpercent2 - bulk_wtpercentH2)/delta, (bulk_wtpercentH2 - wtpercent1)/delta
    def subregular(Tsubreg, bulk_wtpercentH2, PGPa_sub, last_guess=None, lowT_cutoff=0.7):
        if not use_binodal_cache:
            return _subregular_exact(Tsubreg, bulk_wtpercentH2, PGPa_sub, last_guess, lowT_cutoff)
        # mirror the exact solver's input guards so keys are stable
        if (Tsubreg is None) or (not np.isfinite(Tsubreg)):
            Tsubreg = 1500.0
        T_key = max(float(Tsubreg), 1500.0)
        bulk_eff = max(bulk_wtpercentH2, 1.0e-3)
        key = (round(T_key, 1), round(float(PGPa_sub), 4))
        hit = _coex_cache.get(key)
        if hit is not None:
            x1, x2, Tcrit = hit
            ws, wa = _lever_rule(x1, x2, bulk_eff)
            return x1, x2, Tcrit, ws, wa
        x1, x2, Tcrit, ws, wa = _subregular_exact(Tsubreg, bulk_wtpercentH2, PGPa_sub, last_guess, lowT_cutoff)
        _coex_cache[key] = (x1, x2, Tcrit)
        return x1, x2, Tcrit, ws, wa

    if _eos_selftest:
        return {'eos_exact': _EOS_MgSiO3_exact, 'eos_cached': EOS_MgSiO3,
                'subreg_exact': _subregular_exact, 'subreg_cached': subregular}
    # ----------------------------------------------------------
...
#        # ============================================================
        ...
        T_upper=T[n-2]
        # (dead line removed: it referenced best_j, which is only bound inside the RCB-patch block
        #  above, so it raised UnboundLocalError on paths that skip that block. The value was unused
        #  -- the cost below uses r[n-2].)
    ...
    # =====================================================================
    #  NON-EQUILIBRIUM SNAPSHOT MODE  (reuses model/solve_for_structure/rho_0_mix/subregular)
    #  Given (M_sil=Mc, X_int, T0, and either Ps or target M_atm): build ONE
    #  self-consistent structure with NO equilibrium iteration.
    #    * X_int (transport-limited bulk melt H2) sets the core EOS density
    #    * melt-side binodal x1 at (T0,Ps) reported as boundary composition
    #    * a single Ps root-find pins atmosphere mass to M_atm (a state var)
    # =====================================================================
    if snapshot:
        def _set_geom(R_s_v, Mc_v, r_atm_v, RB_v, rmax_v, T_contact_v, Ps_v):
            nonlocal R_s, Mc, r_atm, RB, rmax, T_contact, Ps
            R_s, Mc, r_atm, RB, rmax = R_s_v, Mc_v, r_atm_v, RB_v, rmax_v
            # model() reads raw module T_contact / Ps for base gas composition (subregular)
            T_contact, Ps = T_contact_v, Ps_v

        Xi   = float(snap_X_int)
        T0s  = float(snap_T0_contact)
        Mc_l = float(snap_Msil_kg) if (snap_Msil_kg is not None) else float(Mp_target)
        rho_0s = rho_0_mix(Xi, MW2, MW1, 2.5, 0.09)

        # 1. core solve (radius ~ independent of surface Ps)
        Ps_guess = float(snap_Ps_GPa) if (snap_Ps_GPa is not None) else float(target)
        m4,r4,P4,rho4,Rcmb4,Tcore = solve_for_structure(
            Mc_l, cmf, cmf_mix, xrho, rho_0s, T0s, Ps_guess*1.0e9, structure, wtfracH2=Xi)
        R_s_l = r4[-1]
        if snap_force_Rcore_m is not None:
            R_s_l = float(snap_force_Rcore_m)

        # CORE-ONLY mode: the caller just needs the current interior radius (for the evolving
        # geometry: surface area, gravity, ocean depth). The atmosphere march is skipped, so this
        # costs seconds instead of minutes.
        if snap_core_only:
            return {'X_int': Xi, 'T0_contact_K': T0s, 'rho_0_gcc': rho_0s, 'Mc_kg': Mc_l,
                    'r_core_m': float(R_s_l), 'r_core_Earth': float(R_s_l)/R_earth_meters,
                    'Rcmb_m': float(Rcmb4),
                    'T_central_K': float(Tcore[0]) if len(Tcore) else float('nan'),
                    'core': {'m': m4, 'r': r4, 'P': P4, 'rho': rho4, 'T': Tcore}}

        # 2. atmosphere geometry from this core
        Cs_l = np.sqrt(gamma_gas*kb*Teq/m_main)
        RB_l = 2.0*G*Mc_l/(Cs_l**2)
        rmax_l = RB_l/RB_over_rmax
        r_atm_l = gridding(R_s_l, rmax_l, int(nr), sharp)

        def _atm(Ps_val):
            _set_geom(R_s_l, Mc_l, r_atm_l, RB_l, rmax_l, T0s, Ps_val)
            return model(2, r_atm_l, Ra=Ra, T_contact_in=T0s, Ps_in=Ps_val)

        # 3. Ps: given, or root-find on target M_atm
        if snap_Ps_GPa is not None:
            Ps_final = float(snap_Ps_GPa); atm = _atm(Ps_final)
        else:
            Mtar = float(snap_Matm_kg)
            def _mass_at(Ps_val):
                # M_atm(Ps); returns None where the atmosphere solve fails (e.g. no
                # binodal coexistence at fixed T0 for this Ps).
                try:
                    o = _atm(Ps_val); mv = o[3]
                    return float(mv[len(mv)-2])
                except Exception:
                    return None
            # scan a Ps grid, keep successful points, bracket the target M_atm
            import numpy as _np
            grid = _np.linspace(max(0.3*float(target), 0.5), 6.0*float(target), 18)
            pts = [(pv, _mass_at(pv)) for pv in grid]
            pts = [(pv, mm) for pv, mm in pts if (mm is not None and _np.isfinite(mm))]
            if len(pts) < 2:
                raise RuntimeError("snapshot: could not evaluate M_atm(Ps) on the scan grid")
            # M_atm increases with Ps; find the bracketing interval around Mtar
            br = None
            for a, b in zip(pts[:-1], pts[1:]):
                if (a[1]-Mtar)*(b[1]-Mtar) <= 0:
                    br = (a[0], b[0]); break
            from scipy.optimize import brentq as _brentq
            if br is not None:
                def _resid(Ps_val):
                    mm = _mass_at(Ps_val)
                    return (mm - Mtar) if mm is not None else 1e30
                Ps_final = _brentq(_resid, br[0], br[1], xtol=1e-4, rtol=1e-6, maxiter=40)
            else:
                # target outside scanned range: take nearest successful Ps
                Ps_final = min(pts, key=lambda pm: abs(pm[1]-Mtar))[0]
            atm = _atm(Ps_final)
        ...
        (T,P,density,mass,mass_cond,dPdr_save,MW_gas,MW_melt,m_molec,k_ratio,
         ...
         x_inhib,Sgaskg,Smeltkg,S_sys_kg,tau,Tint,i_rcb_l,Lint_l,delta_bl_l,
         ledoux_top_height_m_l) = atm
        nlast = len(mass); Matm_real = float(mass[nlast-2])
        try:
            x1,x2,Tcrit,ws,wa = subregular(T0s, 100.0*Xi, Ps_final)
        except Exception:
            x1=x2=Tcrit=ws=wa=float('nan')

        # ---- transit (chord) radius: Guillot 2010 Eq.56, scanned from the top
        #      down until the slant optical depth exceeds tau_critical (cheap: the
        #      crossing is in the thin upper atmosphere).  This is R_planet. ----
        def _transit_radius():
            rr = r_atm_l; dens = density; n_c = len(rr)
            drc = np.empty(n_c); drc[1:] = rr[1:]-rr[:-1]; drc[0] = drc[1]
            UNIT = 0.1
            for i in range(n_c-2, 0, -1):
                rp = rr[i]
                dz0 = max(rr[i+1]-rr[i], 0.0)
                ds0 = np.sqrt(dz0*dz0 + 2.0*rp*dz0)
                Nr_tan = 2.0*dens[i]*ds0
                rj = rr[i+1:]; den = np.sqrt(np.maximum(rj*rj - rp*rp, 0.0))
                geom = np.where(den > 0.0, rj/np.where(den>0.0, den, 1.0), 0.0)
                Nr_above = 2.0*np.sum(dens[i+1:]*geom*drc[i+1:])
                tau_c = UNIT*(Nr_tan+Nr_above)*my_kappa(T[i], P[i], 0.0)
                if tau_c > tau_critical:
                    return float(rr[i])
            return float(rr[1])
        try:
            R_transit_m = _transit_radius()
        except Exception:
            R_transit_m = float('nan')

        return {
            'R_transit_m': R_transit_m, 'R_transit_Earth': R_transit_m/R_earth_meters,
            'X_int': Xi, 'T0_contact_K': T0s, 'Ps_GPa': Ps_final, 'rho_0_gcc': rho_0s,
            'Mc_kg': Mc_l, 'r_core_m': R_s_l, 'r_core_Earth': R_s_l/R_earth_meters,
            'Rcmb_m': float(Rcmb4), 'T_central_K': float(Tcore[0]) if len(Tcore) else float('nan'),
            'Matm_kg': Matm_real, 'P_base_bar': float(P[0]/1e5),
            'i_rcb': int(i_rcb_l) if i_rcb_l is not None else -1,
            'Lint': float(Lint_l), 'delta_bl_m': float(delta_bl_l),
            'ledoux_top_height_m': float(ledoux_top_height_m_l),
            'Trcb_K': float(T[i_rcb_l]) if (i_rcb_l is not None) else float('nan'),
            'x1_melt_binodal': float(x1), 'x2_atm_binodal': float(x2), 'Tcrit_K': float(Tcrit),
            'core': {'m': m4, 'r': r4, 'P': P4, 'rho': rho4, 'T': Tcore},
            'atm':  {'T': T, 'P': P, 'rho': density, 'mass': mass, 'r': r_atm_l,
                     'gradT': gradT_saved, 'nabla_rad': k_rad_saved, 'nabla_conv': k_conv_saved,
                     'nabla_cond': k_cond_saved, 'nabla_nc': k_nc_saved, 'tau': tau,
                     'xH2': xH2atmosphere, 'MW_gas': MW_gas, 'm_molec': m_molec,
                     'i_rcb_attach': (int(i_rcb_l) if i_rcb_l is not None else -1)},
        }
    # =====================================================================

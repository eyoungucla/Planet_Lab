...
# Import J2/J4 empirical calibration (ice-giant, all-4-planet fit)
...
# ─────────────────────────────────────────────────────────────────────────────
#  J2 / J4  —  empirical ice-giant calibration (all-4-planet fit)
# ─────────────────────────────────────────────────────────────────────────────
# Use the full planet density/radius profile (r_planet, rho_planet) truncated
# at the 1-bar level, consistent with what the MCMC codes do.
...
# Rotational parameter q using Neptune rotation period (or Teq planet — use SS Neptune as reference)
# We use the actual planet mass and 1-bar radius from the model, with Neptune P_rot
_q_rot = J2mod.compute_q({'P_rot': J2mod.SS['Neptune']['P_rot'],
                           'Rp':    _R1bar,
                           'M':     _M_int})

_coef = J2mod.build_calibration()
_J2_pred, _J4_pred = J2mod.predict_J2_J4(_C_pred, _q_rot, _coef)

# Neptune observed values for comparison
_J2_U_obs = J2mod.SS['Neptune']['J2']
_J4_U_obs = J2mod.SS['Neptune']['J4']
_C_U_obs  = J2mod.SS['Neptune']['C']

_sig_J2, _sig_J4, _frac_J2, _frac_J4 = J2mod.calibration_sigma('Neptune')

j2_lines = [
    "",
    "─" * 62,
    "  GRAVITY HARMONICS  (empirical ice-giant calibration)",
    "─" * 62,
    f"  {'1-bar radius':<28}: {_R1bar/1e3:.1f} km  =  {_R1bar/6.371e6:.4f} R_Earth",
    f"  {'C/MR²  model':<28}: {_C_pred:.4f}",
    f"  {'C/MR²  Neptune obs':<28}: {_C_U_obs:.4f}  (diff: {(_C_pred-_C_U_obs)/_C_U_obs*100:+.2f}%)",
    f"  {'q_rot':<28}: {_q_rot:.6f}",
    "",
    f"  {'J2  model':<28}: {_J2_pred*1e6:+10.3f}  × 10⁻⁶",
    f"  {'J2  Neptune obs':<28}: {_J2_U_obs*1e6:+10.3f}  × 10⁻⁶"
    f"  (cal σ = {_sig_J2*1e6:.2f}e-6,  {_frac_J2*100:.2f}%)",
    f"  {'ΔJ2':<28}: {(_J2_pred-_J2_U_obs)*1e6:+.3f} × 10⁻⁶"
    f"  ({(_J2_pred-_J2_U_obs)/_sig_J2:+.2f} σ)",
    "",
    f"  {'J4  model':<28}: {_J4_pred*1e6:+10.4f}  × 10⁻⁶",
    f"  {'J4  Neptune obs':<28}: {_J4_U_obs*1e6:+10.4f}  × 10⁻⁶"
    f"  (cal σ = {_sig_J4*1e6:.3f}e-6,  {_frac_J4*100:.2f}%)",
    f"  {'ΔJ4':<28}: {(_J4_pred-_J4_U_obs)*1e6:+.4f} × 10⁻⁶"
    f"  ({(_J4_pred-_J4_U_obs)/_sig_J4:+.2f} σ)",
    "─" * 62,
    "  Calibration: J2 = a·q + b·q·C + c·q²  (all 4 SS giants)",
    "  J4 = slope_ice · J2  (mean J4/J2 for Uranus + Neptune)",
    "─" * 62,
]
j2_summary = "\n".join(j2_lines)
print(j2_summary)

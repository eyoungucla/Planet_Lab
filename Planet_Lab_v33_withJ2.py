...
# Import J2/J4 helpers (1-bar radius finder, mass/MOI integration, q_rot)
import calculate_J2 as J2mod

# ── J2/J4 method ──────────────────────────────────────────────────────────────
# RUN_J2 toggles the gravity-harmonic calculation on/off. Set False to skip it
# entirely (e.g. when only the interior/atmosphere profile is needed, or for
# faster iteration) since CMS adds ~30–120 s per run.
# When RUN_J2 is True, J2/J4 are ALWAYS computed with full CMS via
# CMSPlanet/cms.py. Requires run_cms_single.py, cms.py, CMSPlanet.py in the
# same dir. The older empirical calibration path has been removed.
RUN_J2       = True    # ← set False to skip the J2/J4 (CMS) calculation
J2_METHOD    = 'cms'   # fixed; empirical path removed
CMS_N_LAYERS = 256           # CMS shells
CMS_XLAYERS  = -1            # -1 = full calculation (recommended for sharp profiles)
...
label50 = QtWidgets.QLabel('Rotation period (for J2/J4)')
box50 = QtWidgets.QDoubleSpinBox()
box50.setDecimals(4)
box50.setMinimum(0.0001)
box50.setMaximum(1000.0)
box50.setSingleStep(0.1)
box50.setSuffix(" hr")
box50.setValue(17.2400)

label51 = QtWidgets.QLabel('gamma = kappa_vis/kappa_th')
box51 = QtWidgets.QDoubleSpinBox()
box51.setDecimals(4)
box51.setMinimum(0.0001)
box51.setMaximum(100.0)
box51.setSingleStep(0.05)
box51.setValue(1.0000)
...
layout.addWidget(label50, 11, 0)
layout.addWidget(box50, 12, 0)
layout.addWidget(label51, 13, 0)
layout.addWidget(box51, 14, 0)
...
P_rot_hr = box50.value()
P_rot_sec = P_rot_hr * 3600.0
print("Entered rotation period = %8.4f hr  (%.1f s)" % (P_rot_hr, P_rot_sec))

gamma_vis_th_in = box51.value()
print("Entered gamma (kappa_vis/kappa_th) = %8.4f" % gamma_vis_th_in)
...
ledoux_top_height_m = res.ledoux_top_height_m
...
summary_text = legacy_summary_text(res)
print(summary_text)

# ─────────────────────────────────────────────────────────────────────────────
#  J2 / J4  —  computed with full CMS (always), unless RUN_J2 is False
# ─────────────────────────────────────────────────────────────────────────────
if RUN_J2:
    _r_J2   = np.asarray(r_planet,   dtype=float)
    _rho_J2 = np.asarray(rho_planet, dtype=float)
    _P_J2   = np.asarray(P_planet,   dtype=float)   # Pa

    # Find 1-bar radius index
    _P_bar_J2 = _P_J2 / 1.0e5
    _R1bar, _i1bar = J2mod.find_1bar_radius(_r_J2, _P_bar_J2)

    _r_int   = _r_J2[:_i1bar+1]
    _rho_int = _rho_J2[:_i1bar+1]

    # Integrate mass and MOI
    _M_int, _I_int = J2mod.integrate_mass_moi(_r_int, _rho_int)
    _C_pred = _I_int / (_M_int * _R1bar**2)

    # Rotation parameter q from the user-supplied rotation period (entered in the
    # GUI, box50). NOTE: compute_q expects P_rot in seconds here. If your
    # calculate_J2.compute_q was written to take hours, pass P_rot_hr instead.
    _q_rot  = J2mod.compute_q({'P_rot': P_rot_sec,
                                'Rp':    _R1bar,
                                'M':     _M_int})

    import warnings
    from run_cms_single import run_cms
    print("  Running CMS for J2/J4 (this may take 1–2 minutes)...")
    with warnings.catch_warnings():
        warnings.simplefilter('ignore')
        _cms_res = run_cms(
            'model', _r_int, _rho_int,
            R_1bar   = _R1bar,
    ...
    _J2_pred = _cms_res['J2']
    _J4_pred = _cms_res['J4']
    _method_label = "CMS (Hubbard 2013 / Movshovitz 2019)"

    j2_lines = [
        "",
        "─" * 62,
        f"  GRAVITY HARMONICS  ({_method_label})",
        "─" * 62,
        f"  {'1-bar radius':<28}: {_R1bar/1e3:.1f} km  =  {_R1bar/6.371e6:.4f} R_Earth",
        f"  {'rotation period':<28}: {P_rot_hr:.4f} hr",
        f"  {'q_rot':<28}: {_q_rot:.6f}",
        f"  {'C/MR²  model':<28}: {_C_pred:.4f}",
        "",
        f"  {'J2  model':<28}: {_J2_pred*1e6:+10.3f}  × 10⁻⁶",
        f"  {'J4  model':<28}: {_J4_pred*1e6:+10.4f}  × 10⁻⁶",
        "─" * 62,
        f"  Method: {_method_label}",
        f"  CMS shells: {CMS_N_LAYERS},  xlayers: {CMS_XLAYERS}",
        "─" * 62,
    ]
    j2_summary = "\n".join(j2_lines)
    print(j2_summary)
else:
    print("  RUN_J2 = False → skipping J2/J4 (CMS) calculation.")
    j2_summary = "\n(J2/J4 calculation skipped: RUN_J2 = False)"

# ─────────────────────────────────────────────────────────────────────────────
#  Write SUMMARY.txt
# ─────────────────────────────────────────────────────────────────────────────
summary_path = Path(res.output_dir) / f"SUMMARY{suffix}.txt"
with open(summary_path, 'w') as _fp:
    _fp.write(summary_text + "\n")
    _fp.write(j2_summary + "\n")
...
ymax=1.2*r_planet[i_chord_p]/R_s
ymin=1.0
